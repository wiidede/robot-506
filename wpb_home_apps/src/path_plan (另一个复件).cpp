...
void ProcColorCB(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    if(nState == STATE_DETECT)
    {
        ...
        if(rj==0)
        {
            imwrite("/home/robot/objects1.jpg",cv_ptr->image);
            rj++;
        }
        else if(rj==1)
        {
            imwrite("/home/robot/objects2.jpg",cv_ptr->image);
            rj++;
        }
        else if(rj==2)
        {
            imwrite("/home/robot/objects3.jpg",cv_ptr->image);
            rj++;
        }
        else if(rj==3)
        {
            imwrite("/home/robot/objects4.jpg",cv_ptr->image);
            rj++;
        }
//find_star();作检测
//重复订单 和 饮料

...
#特征匹配初始化
MIN_MATCH_COUNT = 10
temp = cv2.imread('/home/robot/catkin_ws/src/dd_demo/OpenPose/images/model.jpg',0)
template = cv2.resize(temp, (72, 128), interpolation=cv2.INTER_CUBIC)
    ...
    if state == 2:
        ...
        if state == 1:
            if points[1] is not None:
                if points[3] is not None and points[6] is not None :
                    #hangFlag = 1
                    if points[4] is not None or points[7] is not None:
                        hangFlag = 1
        if hangFlag:
            state = 2
            pub.publish('hang up')
            print('detected hanging up the hand')
